...
from transactions.importer import ImportResult, StatementImportError, import_statement
from transactions.models import Account, StatementUpload, Transaction
...
@pytest.fixture
def user():
    return User.objects.create_user(username="test", password="test-pass-123")
...
@pytest.fixture
def make_upload(account, user):
    """Return a factory that saves `data` as a new StatementUpload."""

    def _make(data: bytes, filename: str = "statement.csv") -> StatementUpload:
        upload = StatementUpload(account=account, user=user)
...
def test_import_creates_every_parsed_transaction(make_upload, sample_bytes, account):
    upload = make_upload(sample_bytes)
...
def test_import_marks_the_upload_processed(make_upload, sample_bytes):
    upload = make_upload(sample_bytes)
...
def test_imported_transactions_link_back_to_their_upload(make_upload, sample_bytes):
    upload = make_upload(sample_bytes)
...
def test_fields_are_mapped_onto_the_model(make_upload, sample_bytes, account):
    import_statement(make_upload(sample_bytes))

    gym = Transaction.objects.get(account=account, merchant="SAMPLE GYM")

    assert gym.amount == Decimal("-1.00")
    assert gym.posted_date.isoformat() == "2026-07-31"
    assert gym.transaction_date.isoformat() == "2026-07-30"
...
def test_reimporting_the_same_file_creates_nothing(make_upload, sample_bytes, account):
    import_statement(make_upload(sample_bytes))

    second = import_statement(make_upload(sample_bytes))
...
def test_identical_transactions_are_stored_separately(
    make_upload, sample_bytes, account
):
    """Two real £3.20 coffees on the same day must survive as two rows.

    They differ only by occurrence index, so a naive fingerprint over
    (date, amount, description) would silently discard the second.
    """
    import_statement(make_upload(sample_bytes))
...
def test_the_same_account_can_import_a_second_distinct_statement(
    ...
    result = import_statement(make_upload(extra, "august.csv"))

    assert result == ImportResult(parsed=1, created=1)
    assert Transaction.objects.filter(account=account).count() == 13


def test_unrecognised_format_fails_the_upload(make_upload, account):
    upload = make_upload(b"Foo,Bar,Baz\n1,2,3\n")
...
def test_a_single_bad_row_prevents_the_whole_import(make_upload, account):
